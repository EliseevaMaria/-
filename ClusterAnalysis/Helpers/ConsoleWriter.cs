...
namespace ClusterAnalysis.Helpers
{
    public static class ConsoleWriter
    {
        public static void WriteCoutryStatistics(CountryStatistics[] countryStatistics)
        {
            ...
            var maxSugarLength = countryStatistics.Max(stat => stat.AlcoholConsumption.ToString().Length) + 2;
            var maxDeathLength = countryStatistics.Max(stat => stat.FruitConsumption.ToString().Length) + 2;
            ...
            foreach (var countryStats in countryStatistics)
            {
                ...
                int sugarLength = countryStats.AlcoholConsumption.ToString().Length;
                message.Append(countryStats.AlcoholConsumption);
                ConsoleWriter.AppendSpaces(message, maxSugarLength - sugarLength);

                int deathLength = countryStats.FruitConsumption.ToString().Length;
                message.Append(countryStats.FruitConsumption);
